...
# Import Google Generative AI for personal Gemini key
try:
    import google.generativeai as genai
    GEMINI_DIRECT_AVAILABLE = True
    print("✅ Google Generative AI loaded - Personal Gemini key available")
except ImportError:
...
# Configuration Gemini with personal key ONLY
if GEMINI_API_KEY and GEMINI_DIRECT_AVAILABLE:
...
# =========================
# GOOGLE GEMINI INTEGRATION - CLÉ PERSONNELLE GRATUITE UNIQUEMENT
# =========================
async def generate_enhanced_content_with_gemini(passage: str, rubric_type: str, base_content: str = "", content_length: int = 500) -> str:
    """
    Utilise votre clé Gemini personnelle GRATUITE pour enrichir le contenu théologique
    """
    if not GEMINI_AVAILABLE or not GEMINI_API_KEY:
        print("⚠️ Personal Gemini not available, using base content")
        return base_content
    ...
    try:
        ...
        if rubric_type == "verse_by_verse":
            if content_length <= 500:
                ...
                verse_length_instruction = "équilibrée (150-250 mots par verset)"
            else:
                verse_length_instruction = "détaillée (300-500 mots par verset)"
            
            prompt = f"""
Génère une étude biblique verset par verset pour {passage} avec analyse {length_instruction}.

Instructions spécifiques :
- Pour CHAQUE verset, fournis OBLIGATOIREMENT cette structure :
  VERSET [numéro] 📖
  TEXTE BIBLIQUE 📜 : [texte exact du verset]
  EXPLICATION THÉOLOGIQUE 🎓 : [analyse {verse_length_instruction}]

- L'explication doit être {detail_level}
- Reste fidèle au texte biblique
- Utilise un français théologique précis
- Longueur totale cible : {content_length} caractères

{base_content if base_content else ""}
"""
        
        else:  # Pour les autres rubriques
            prompt = f"""
Génère du contenu théologique pour la rubrique "{rubric_type}" sur le passage {passage}.

Contenu requis :
- Analyse {detail_level}
- Style : {length_instruction}
- Longueur cible : {content_length} caractères
- Français théologique précis

{base_content if base_content else ""}
"""
        
        # Utiliser votre modèle Gemini gratuit
        model = genai.GenerativeModel('gemini-2.0-flash-exp')
        response = model.generate_content(prompt)
        
        if response and response.text:
            result = response.text.strip()
            print(f"✅ Content generated with personal Gemini key (GRATUIT): {len(result)} chars")
            return result
        else:
            print("⚠️ Empty response from personal Gemini")
            return base_content
            
    except Exception as e:
        print(f"❌ Error with personal Gemini key: {e}")
        return base_content
...
async def generate_simple_theological_explanation(verse_text: str, book_name: str, chapter: int, verse_num: int, content_length: int = 500) -> str:
    """
    Génère une explication théologique spécifique pour chaque verset en utilisant votre clé Gemini gratuite
    """
    # Si votre Gemini est disponible, générer une explication spécifique
...
"""
            ...
            if response and response.text:
                ...
                if len(explanation) > 50:  # Vérifier que la réponse est substantielle
                    print(f"✅ Personal Gemini generated specific explanation for {book_name} {chapter}:{verse_num} (GRATUIT)")
                    return explanation
            
        except Exception as e:
            print(f"⚠️ Personal Gemini explanation failed for {book_name} {chapter}:{verse_num}: {e}")
    else:
        print(f"⚠️ Personal Gemini not available for {book_name} {chapter}:{verse_num}")
