...
function App() {
  ...
  // Fonction pour enrichir un verset spécifique avec Gemini
  const enrichirVerset = async (livre, chapitre, verset) => {
    try {
      console.log(`[ENRICHIR VERSET] ${livre} ${chapitre}:${verset}`);
      
      const passage = `${livre} ${chapitre}:${verset}`;
      const payload = {
        passage: passage,
      ...
      // Indiquer que l'enrichissement est en cours
      const verseElement = document.querySelector(`[data-verse="${verset}"]`);
      if (verseElement) {
        const button = verseElement.querySelector('.btn-gemini-verse');
        if (button) {
          button.innerHTML = '⏳ Enrichissement...';
          button.disabled = true;
        }
      }

      const { data, url } = await smartPost(ENDPOINTS.verseGemini, payload);
      console.log("[ENRICHIR VERSET OK]", url);

      // Remplacer le contenu du verset avec la version enrichie
      if (verseElement && data.content) {
        const enrichedContent = postProcessMarkdown(data.content);
        const parser = new DOMParser();
        const doc = parser.parseFromString(enrichedContent, 'text/html');
        
        // Extraire l'explication théologique enrichie
        const explanationMatch = enrichedContent.match(/EXPLICATION THÉOLOGIQUE\s*:(.*?)(?=VERSET|\s*$)/s);
        if (explanationMatch) {
          const enrichedExplanation = explanationMatch[1].trim();
          const explanationElement = verseElement.querySelector('.explication-label').nextElementSibling;
          if (explanationElement) {
            explanationElement.innerHTML = enrichedExplanation.replace(/\n/g, '<br>');
          }
        }
        ...
        const button = verseElement.querySelector('.btn-gemini-verse');
        if (button) {
          button.innerHTML = '✅ Enrichi avec Gemini';
      ...
      console.error("Erreur enrichissement verset:", error);
      const verseElement = document.querySelector(`[data-verse="${verset}"]`);
      if (verseElement) {
        const button = verseElement.querySelector('.btn-gemini-verse');
        if (button) {
  ...
  React.useEffect(() => {
    window.enrichirVerset = enrichirVerset;
    return () => {
      delete window.enrichirVerset;
    };
  }, [selectedBook, selectedChapter, selectedVersion, selectedLength]);
  ...
  const generateVerseByVerseProgressive = async () => {
    try {
      ...
      // 🔹 CORRECTION : Passer les paramètres de longueur à l'API
      console.log(`[VERSETS PROG] Génération progressive pour: ${passage} avec longueur: ${selectedLength}`);

      // 🔹 UTILISER L'API RAILWAY avec les bons paramètres
      console.log("[VERSETS PROG] Utilisation API Railway avec paramètres de longueur");
      ...
      const requestBody = {
        passage: passage,
        version: selectedVersion || 'LSG',
        tokens: parseInt(selectedLength) || 500,  // 🔹 CORRECTION : Passer selectedLength
        use_gemini: true,  // 🔹 CORRECTION : Activer Gemini pour enrichissement
        enriched: true
      };
      ...
      if (data.content) {
        // Affichage immédiat du contenu optimisé (5 versets + boutons Gemini)
        ...
        console.log("[SUCCESS] Contenu affiché avec boutons Gemini");
      } else {
  ...
  const generateWithGemini = async () => {
    try {
      ...
      // 🔹 CORRECTION : Passer les paramètres de longueur
      const payload = activeRubrique === 0
        ? { 
            passage, 
            version: selectedVersion, 
            tokens: parseInt(selectedLength) || 500,  // 🔹 CORRECTION
            use_gemini: true,
        ...
        : { 
            ...
            tokens: parseInt(selectedLength) || 500,  // 🔹 CORRECTION
            use_gemini: true,
            requestedRubriques: [activeRubrique], 
            enriched: true 
          };
      ...
      for (let rubriqueIndex = 0; rubriqueIndex < totalRubriques; rubriqueIndex++) {
        ...
        try {
          ...
          const { data, url } = await smartPost(ENDPOINTS.studyGemini, {
            ...
            version: selectedVersion || 'LSG',
            tokens: parseInt(selectedLength) || 500,  // 🔹 CORRECTION: Passer selectedLength
            use_gemini: true,  // 🔹 CORRECTION: Activer Gemini
  ...
  const formatContent = (text, context = 'default') => {
    ...
    // Formatage avec contexte pour VERSETS PROG avec boutons Gemini
    ...
    // 🔹 NOUVEAU: Traitement spécial pour les versets avec boutons Gemini
    if (context === 'verse-by-verse' || text.includes('VERSET')) {
      // Diviser le texte en sections de versets
      const sections = formattedText.split(/(VERSET \d+)/);
      let processedHtml = '';
      
      for (let i = 0; i < sections.length; i++) {
        const section = sections[i];
        
        // Si c'est un titre de verset
        if (section.match(/VERSET (\d+)/)) {
          const verseNumber = section.match(/VERSET (\d+)/)[1];
          processedHtml += `<h2 class='verset-header'>📖 VERSET ${verseNumber}</h2>`;
          
          // Traiter la section suivante (contenu du verset)
          if (i + 1 < sections.length) {
            let verseContent = sections[i + 1];
            
            // Formater le contenu du verset
            ...
            const paragraphs = verseContent.split("\n\n")
              .map(p => (p.trim() ? `<p>${p.replace(/\n/g, "<br>")}</p>` : ""))
              .filter(p => p)
              .join("");
            
            processedHtml += `
              <div class="verse-content" data-verse="${verseNumber}">
                ${paragraphs}
                <div class="verse-actions" style="margin: 15px 0; text-align: center;">
                  <button 
                    class="btn-gemini-verse" 
                    onclick="enrichirVerset('${selectedBook}', '${selectedChapter}', '${verseNumber}')"
                    style="
            ...
            i++; // Passer la section suivante car on l'a déjà traitée
          }
        } else if (!section.match(/VERSET \d+/) && section.trim()) {
          // Autres contenus (titre, introduction, etc.)
          const formatted = section.split("\n\n")
            .map(p => (p.trim() ? `<p>${p.replace(/\n/g, "<br>")}</p>` : ""))
            .join("");
          processedHtml += formatted;
        }
      }
    ...
    // Formatage standard pour autres contextes
    const standardFormatted = formattedText
    ...
    if (context === 'versets-prog') {
      return `<div class="versets-prog-content">${standardFormatted}</div>`;
    }
          ...
          <div className="center-column">
            ...
            <div className="content-area">
              {isLoading ? (
                ...
                <div className="content-text" dangerouslySetInnerHTML={{ __html: formatContent(content, isVersetsProgContent ? 'verse-by-verse' : 'default') }} />
              ) : (
