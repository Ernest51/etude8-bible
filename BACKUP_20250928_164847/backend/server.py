...
# =========================
async def generate_enhanced_content_with_gemini(passage: str, rubric_type: str, base_content: str = "", content_length: int = 500) -> str:
    """
    Utilise Google Gemini Flash pour enrichir le contenu théologique
    avec une longueur adaptée selon le paramètre content_length
    """
    ...
    try:
        # Déterminer la longueur cible selon le paramètre
        if content_length <= 500:
            length_instruction = "concise (200-300 mots par verset)"
            detail_level = "essentiel"
        elif content_length <= 1500:
            length_instruction = "détaillée (400-600 mots par verset)"
            detail_level = "approfondie avec contexte historique"
        else:
            length_instruction = "très détaillée (800-1200 mots par verset)"
            detail_level = "exhaustive avec références croisées, contexte historique, culturel et applications"
        
        # Initialiser le chat avec Gemini Flash
        chat = LlmChat(
            api_key=EMERGENT_LLM_KEY,
            session_id=f"bible_study_{passage.replace(' ', '_')}_{content_length}",
            system_message=(
                f"Tu es un théologien expert spécialisé dans l'étude biblique approfondie. "
                f"Tu génères des contenus théologiques de niveau {detail_level} en français. "
                f"Tes explications sont {length_instruction}, spirituellement édifiantes et toujours fidèles au texte biblique. "
                f"Adapte la profondeur de ton analyse selon la longueur demandée : {content_length} caractères cible."
            )
        ).with_model("gemini", "gemini-2.0-flash")
        
        # Créer le prompt selon le type de rubrique ET la longueur demandée
        if rubric_type == "verse_by_verse":
            # Ajuster les instructions selon la longueur pour éviter les contradictions
            if content_length <= 500:
                verse_length_instruction = "concise (80-120 mots par verset)"
                total_instruction = f"Assure-toi que le contenu total reste sous {content_length} caractères"
            elif content_length <= 1500:
                verse_length_instruction = "détaillée (150-250 mots par verset)"
                total_instruction = f"Adapte le nombre de versets selon la limite de {content_length} caractères"
            else:
                verse_length_instruction = "très détaillée (300-500 mots par verset)"
                total_instruction = f"Génère un contenu riche jusqu'à {content_length} caractères"
            
            prompt = f"""
Génère une étude théologique verset par verset pour le passage biblique : {passage}

INSTRUCTIONS DE LONGUEUR :
- Explication par verset : {verse_length_instruction}
- Contrainte totale : {total_instruction}
- Niveau de détail : {detail_level}

Pour chaque verset :
1. **TEXTE BIBLIQUE** : Cite le verset exact
2. **EXPLICATION THÉOLOGIQUE** : Analyse {detail_level} incluant :
   - Contexte historique et culturel
   - Signification théologique
   - Applications spirituelles
   - Références croisées pertinentes (si longueur > 500)
   - Analyse lexicale des mots clés (si longueur > 1500)
...
[analyse selon {verse_length_instruction}]

{total_instruction}.
"""
        elif rubric_type == "thematic_study":
            prompt = f"""
Génère une étude thématique {length_instruction} pour le passage biblique : {passage}

INSTRUCTIONS DE LONGUEUR :
- Niveau de détail : {detail_level}
- Longueur totale visée : {content_length} caractères
- Adaptation de chaque section selon cette longueur

Structure requise avec les 28 rubriques d'étude biblique (adaptées à la longueur {content_length}) :
...
Chaque section doit être adaptée à la longueur {content_length} et au niveau {detail_level} spécifiquement pour le passage {passage}.
"""
        else:
            prompt = f"Génère un contenu théologique enrichi ({length_instruction}) pour le passage {passage} sur le thème : {rubric_type}. Sois {detail_level} et spirituellement édifiant selon la longueur cible de {content_length} caractères."
...
# =========================
async def _adapt_content_length(content: str, target_length: int, passage: str) -> str:
    """
    Adapte le contenu selon la longueur cible demandée
    """
    if target_length <= 500:
        # Version concise : garder l'essentiel
        return content
    elif target_length <= 1500:
        # Version moyenne : ajouter du contexte
        enhanced_content = content + f"\n\n**CONTEXTE ENRICHI pour {passage}**\n\n"
        enhanced_content += "Cette étude révèle la richesse du texte biblique dans son contexte historique et théologique. "
        enhanced_content += "Chaque verset s'inscrit dans la révélation progressive de Dieu et trouve sa signification pleine dans l'ensemble de l'Écriture."
        return enhanced_content
    else:
        # Version détaillée : maximum de contenu
        enhanced_content = content + f"\n\n**ANALYSE THÉOLOGIQUE APPROFONDIE de {passage}**\n\n"
        enhanced_content += "Cette étude exhaustive explore les dimensions historiques, culturelles, linguistiques et spirituelles du passage. "
        enhanced_content += "Le contexte original, les parallèles bibliques et les applications contemporaines enrichissent notre compréhension. "
        enhanced_content += "L'analyse lexicale des termes originaux (hébreu/grec) révèle les nuances perdues dans la traduction. "
        enhanced_content += "Les références croisées dans l'ensemble des Écritures confirment et approfondissent les vérités révélées ici."
        return enhanced_content

async def generate_simple_theological_explanation(verse_text: str, book_name: str, chapter: int, verse_num: int, content_length: int = 500) -> str:
    """
    Génère une explication théologique spécifique pour chaque verset en utilisant Gemini Flash
    avec adaptation selon la longueur demandée
    """
    # Si Gemini est disponible, générer une explication spécifique
    if GEMINI_AVAILABLE and EMERGENT_LLM_KEY:
        try:
            # Déterminer le niveau de détail selon la longueur
            if content_length <= 500:
                detail_instruction = "concise (80-120 mots)"
                detail_level = "essentiel"
            elif content_length <= 1500:
                detail_instruction = "détaillée (150-250 mots)"
                detail_level = "avec contexte historique"
            else:
                detail_instruction = "très détaillée (300-400 mots)"
                detail_level = "exhaustive avec références croisées"
            
            # 🔹 OPTIMISATION: Timeout plus court pour éviter les blocages
            # Créer un prompt spécifique pour ce verset
            prompt = f"""
Génère une explication théologique {detail_instruction} pour ce verset biblique :

**Verset** : {book_name} {chapter}:{verse_num}
**Texte** : {verse_text}

INSTRUCTIONS :
- Fournis une explication théologique {detail_level}
- Longueur cible : {detail_instruction}
- Sois spécifique au contenu exact de ce verset (pas générique)
- Inclus le contexte historique ou culturel pertinent (si longueur > 500)
- Inclus des références croisées (si longueur > 1500)
...
"""
            ...
            # Utiliser Gemini pour générer l'explication AVEC TIMEOUT
            import asyncio
            
            async def generate_with_timeout():
                chat = LlmChat(
                    api_key=EMERGENT_LLM_KEY,
                    session_id=f"verse_explanation_{book_name}_{chapter}_{verse_num}_{content_length}",
                    system_message=f"Tu es un théologien expert qui génère des explications bibliques {detail_level} en français."
                ).with_model("gemini", "gemini-2.0-flash")
            ...
            # Timeout de 10 secondes maximum par verset
            response = await asyncio.wait_for(generate_with_timeout(), timeout=10.0)
            ...
            print(f"⏰ Gemini timeout for {book_name} {chapter}:{verse_num} - using fallback")
        except Exception as e:
    ...
    return _generate_fallback_explanation(verse_text, book_name, chapter, verse_num, content_length)

def _generate_fallback_explanation(verse_text: str, book_name: str, chapter: int, verse_num: int, content_length: int = 500) -> str:
    """
...
async def generate_verse_by_verse(request: StudyRequest):
    ...
    et adaptation selon les paramètres de longueur
    """
    try:
        passage = request.passage.strip()
        # 🔹 OPTIMISATION: Désactiver Gemini par défaut pour les chapitres entiers (performance)
        use_gemini = getattr(request, 'use_gemini', False)  # Par défaut False pour éviter les timeouts
        content_length = getattr(request, 'tokens', 500)  # Récupérer la longueur demandée
        
        if not passage:
            raise HTTPException(status_code=400, detail="Passage requis")
        
        print(f"🚀 Generating verse-by-verse for {passage} with length={content_length}, gemini={use_gemini}")
        
        # Générer le contenu de base
        base_content = await _generate_verse_by_verse_content(request)
        
        # Si Gemini est demandé ET activé explicitement, enrichir le contenu avec la longueur appropriée
        if use_gemini and GEMINI_AVAILABLE:
            print(f"🚀 Enhancing verse-by-verse with Gemini Flash for {passage} (length: {content_length})")
            ...
            # Utiliser le contenu de base adapté à la longueur (plus rapide)
            adapted_content = await _adapt_content_length(
                base_content.get("content", ""), 
                content_length, 
                passage
            )
...
async def _generate_verse_by_verse_content(req):
    """Génère le contenu verset par verset de base avec adaptation de longueur"""
    ...
    # Récupérer la longueur demandée
    content_length = getattr(req, 'tokens', 500)

    title = f"**Étude Verset par Verset - {book_label} Chapitre {chap}** (Niveau: {content_length} caractères)"
    
    # Adapter l'introduction selon la longueur
    if content_length <= 500:
        intro = "Étude concise du texte biblique avec explications essentielles."
    elif content_length <= 1500:
        intro = "Étude approfondie avec contexte historique et applications spirituelles enrichies."
    else:
        intro = "Étude exhaustive incluant analyse lexicale, références croisées et applications détaillées."
    
    intro += " Les sections *EXPLICATION THÉOLOGIQUE* sont générées automatiquement par IA théologique."

    if verse:
        # Générer l'explication théologique pour le verset unique avec longueur adaptée
        theological_explanation = await generate_simple_theological_explanation(text, book_label, chap, verse, content_length)
    ...
    # 🔹 OPTIMISATION AGRESSIVE: Limiter drastiquement le nombre de versets
    max_verses = 5 if content_length <= 500 else 7 if content_length <= 1500 else 10
    print(f"[OPTIMISATION] Limitation à {max_verses} versets pour {book_label} {chap}")
    
    verse_count = 0
    for line in lines:
        if verse_count >= max_verses:
            # Message plus clair pour l'utilisateur
            total_verses = len([l for l in lines if re.match(r"^(\d+)\.\s*(.*)$", l)])
            remaining = total_verses - max_verses
            blocks.append(f"\n\n**📚 CHAPITRE COMPLET DISPONIBLE**\n\n✅ **{max_verses} versets** générés avec explications (mode rapide)\n\n🚀 **{remaining} versets restants** disponibles :\n- Utilisez le bouton **'GEMINI FLASH'** pour un chapitre complet enrichi\n- Ou sélectionnez des versets individuels (ex: {book_label} {chap}:12)\n- Les boutons **🤖 Enrichir avec Gemini** permettent d'approfondir chaque verset individuellement")
            break
        ...
        # Générer l'explication théologique pour CHAQUE verset avec la longueur appropriée
        theological_explanation = await generate_simple_theological_explanation(vtxt, book_label, chap, vnum, content_length)
...
async def generate_study(request: StudyRequest):
    ...
    avec adaptation selon les paramètres de longueur
    """
    try:
        passage = request.passage.strip()
        use_gemini = getattr(request, 'use_gemini', True)  # Activer Gemini par défaut
        content_length = getattr(request, 'tokens', 500)  # Récupérer la longueur demandée
        
        if not passage:
            raise HTTPException(status_code=400, detail="Passage requis")
        
        print(f"🚀 Generating study for {passage} with length={content_length}, gemini={use_gemini}")
        
        # Générer le contenu de base avec le système intelligent existant
        base_response = await _generate_intelligent_study(request)
        
        # Si Gemini est demandé, enrichir le contenu avec la longueur appropriée
        if use_gemini and GEMINI_AVAILABLE:
            print(f"🚀 Enhancing with Gemini Flash for {passage} (length: {content_length})")
            enhanced_content = await generate_enhanced_content_with_gemini(
                passage=passage,
            ...
            # Même sans Gemini, adapter la longueur du contenu de base
            adapted_content = await _adapt_content_length(
